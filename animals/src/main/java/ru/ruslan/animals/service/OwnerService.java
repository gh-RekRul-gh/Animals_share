...
public class OwnerService {
    ...
    public Owner findByIdOrElseThrow(long ownerId) {
        return ownerRepository.findByIdWithAnimals(ownerId)
                .orElseThrow(() -> {
                    String message = String.format("Owner not found with id: %d", ownerId);
                    log.warn(message);
    ...
    public void register(OwnerRegisterDto ownerRegisterDto) {
        ...
        if (ownerRepository.existsByEmail(newUserEmail)) {
            String message = String.format("Email already exists: %s", newUserEmail);
            log.warn(message);
